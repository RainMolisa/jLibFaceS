...
if(__name__=='__main__'):
 from evConfig import visThres,\
  ...
  detectModel,testImgFlr,tstImg,dtBufStr,testImgRes
 ...
 net=net.to(device)
 net.eval()
 print('Finished loading model:',detectModel)
 tImgRes=dtBufStr+'/'+testImgRes
 if os.path.exists(dtBufStr):
  shutil.rmtree(dtBufStr)
 os.makedirs(dtBufStr)
 os.makedirs(tImgRes)
 for tstIgI,tstIgStr in enumerate(tstImg):
  testImag=testImgFlr+tstIgStr
  ...
  cv2.imwrite(tImgRes+'/t%03d.jpg'%tstIgI,imgShw)
